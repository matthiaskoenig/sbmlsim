...
def plot_results(xres):
    ...
    for ax in (ax1, ax3):
        add_line(ax=ax, xres=xres,
                 xid='time', yid="X", label="X", xunit=unit_time,
                 yunit=unit_value)
        add_line(ax=ax, xres=xres,
                 xid='time', yid="Y", label="Y", color="darkblue",
                 xunit=unit_time, yunit=unit_value)
        add_line(ax=ax, xres=xres,
                 xid='time', yid="Z", label="Z", color="darkorange",
                 xunit=unit_time, yunit=unit_value)

    for ax in (ax2, ax4):
        add_line(ax=ax, xres=xres, xid="X", yid="Y", label="Y~X", color="black",
                 xunit=unit_value, yunit=unit_value)
