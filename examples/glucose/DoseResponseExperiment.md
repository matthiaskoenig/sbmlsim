...
## Simulations
* [results/DoseResponseExperiment_sim_glc_scan.h5](results/DoseResponseExperiment_sim_glc_scan.h5)
...
class DoseResponseExperiment(SimulationExperiment):
    ...
    def models(self) -> dict[str, AbstractModel]:
        return {
            "model1": RoadrunnerSBMLModel(
                source="model/liver_glucose.xml", base_path=self.base_path
            )
        }
    ...
    def simulations(self) -> dict[str, ScanSim]:
        ...
        glc_scan = ScanSim(
            ...
            dimensions=[
                Dimension(
                    "dim1",
                    changes={"[glc_ext]": self.Q_(np.linspace(2, 20, num=10), "mM")},
                ),
            ],
        )
