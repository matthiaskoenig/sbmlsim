    ...
    def read_sedml(self) -> tuple[libsedml.SedDocument, SEDMLInputType]:
        ...
        if isinstance(self.source, str) and "<sedML" in self.source:
            ...
            try:
                ...
                logger.error("SED-ML string is not valid XML: '%s'", sedml_str)
                raise err
            ...
            if zipfile.is_zipfile(file_path):
                logger.warning("SED-ML from archive: %s", file_path)
                ...
                for entry in sedml_entries:
                    ...
                    logger.error(
                        "No SED-ML file with master flag found in archive: '%s'. Using first file.",
                        file_path,
                    )
                ...
                logger.warning("SED-ML from file: %s", file_path)
                input_type = SEDMLInputType.SEDML_FILE
                if file_suffix not in [".sedml", ".xml"]:
                    logger.error(
                        "SEDML should have [.sedml|.xml] extension: '%s'", file_path
                    )
