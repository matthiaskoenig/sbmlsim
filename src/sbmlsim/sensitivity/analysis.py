...
class SensitivityAnalysis:
    ...
    def samples_table(self, d: dict[str, xr.DataArray]) -> pd.DataFrame:
        return self._data_table(d=self.samples)

    def results_table(self, d: dict[str, xr.DataArray]) -> pd.DataFrame:
        return self._data_table(d=self.results)

    def sensitivity_table(self, d: dict[str, xr.DataArray]) -> pd.DataFrame:
        items = []
        for key, d in self.sensitivity:
            for group in self.groups:
    ...
    def results_table(self) -> pd.DataFrame:
        items = []
        for group in self.groups:
            samples: xr.DataArray = self.samples[group.uid]
            item = {
                'group': group.uid,
                # 'group_name': group.name,
                **samples.sizes,
...
class SamplingSensitivityAnalysis(SensitivityAnalysis):
    ...
    def run_sensitivity_analysis(
        ...
        sa.create_samples()
        console.print(sa.data_table(sa.samples))
        ...
        sa.calculate_sensitivity(cache_filename=f"sensitivity_N{sa.N}.pkl", cache=cache_sensitivity)
