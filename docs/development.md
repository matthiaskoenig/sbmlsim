...
4. bump the version: `uvx bump-my-version bump [major|minor|patch]`, which updates `src/sbmlsim/__init__.py` and `CITATION.cff`, commits and tags
5. `git push --tags`, which triggers the release workflow publishing to [pypi](https://pypi.org/project/sbmlsim/), followed by `git push`
