...
**`fit/` — parameter fitting.** `FitExperiment` names an experiment class and its mappings, `FitMapping` pairs a reference `FitData` (dataset) with an observable `FitData` (task), `FitParameter` carries bounds and unit. `OptimizationProblem` (`fit/optimization.py`) is picklable; `initialize(settings)` creates the runner, resolves the data and the weights and is a no-op when the `FitSettings` did not change, `residuals`/`cost_least_square` are the objective, `optimize` runs scipy least squares or differential evolution. `FitSettings` (`fit/options.py`, which also enumerates residual, loss and weighting types) is the bundle both a fit and its report need. `fit/runner.py` samples start points (`fit/sampling.py`), runs in parallel and shows the progress of the runs with rich; `fit/result.py` collects the fits into an `OptimizationResult`, which stores the settings and hands out the fitted parameters as the `ParameterSets` of `fit/parameters.py`. Reporting is separate from optimizing: `FitReport` (`fit/report.py`) is built from a problem, its settings and one or more parameter sets, so a report is created from stored parameters without running a fit and several sets are compared in one report. Its `index.html` is rendered from the jinja2 template `resources/templates/fit_report.html` into the sections overview, results and fit mappings; the page carries its own css and javascript (search, filter chips, sortable tables, click to enlarge) and loads nothing from the network. The `MappingKind` of a `FitExperiment` decides what a fit does with the mappings it selects: `TRAINING` enters the cost, `VALIDATION` is simulated and evaluated but not fitted, `OUTLIER` is skipped by the problem. The kind belongs to the selection of the data, not to the fit mappings of a simulation experiment, whose `MappingMetaData` describes the curve; `helpers.fit_experiments_by_kind` selects and classifies in one step. `fit/metrics.py` (the former `rmse.py`) calculates the metrics of a parameter set on a problem, i.e., the `DV`/`PRED`/`IPRED` table with the residuals and MSE, RMSE, R² and AIC per mapping, per kind and overall; a report writes them as TSV. `fit/display.py` renders the console output of a fit as sections (the problem, the parameters, the settings and the data with its kinds), so that the runner, the command line tools and an interactive session look the same. `fit/cli.py` holds what every fit shares: a `FitDefinition` is the fit experiments, the parameters, the paths and the settings of a problem, `run_fit` builds the problems for an `OptimizationStrategy` and runs them, and `fit_cli`/`report_cli` are the command line tools a model reuses, so `examples/hctz/fitting/fitting.py` is only the definitions. `fit/petab_omex.py` packages PEtab problems as archives. `examples/hctz` is the reference problem, `tests/fit/` runs against it.
